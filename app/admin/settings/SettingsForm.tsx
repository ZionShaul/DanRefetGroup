...
export default function SettingsForm({ url, enabled }: { url: string; enabled: boolean }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    setMessage(null);
    const res = await updateClickSense(new FormData(e.currentTarget));
    setPending(false);
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-base font-semibold text-brand-ink">קישור למערכת עידן חדש</h2>
        <p className="mt-1 text-sm text-brand-muted">
          הקישור ייפתח בלשונית חדשה דרך הכפתור &quot;נתונים היסטוריים וניתוח&quot;. אם הכפתור מבוטל -
          הוא לא יוצג למשתמשים.
        </p>
      </div>
      ...
      <label className="flex items-center gap-2">
        ...
        />
        <span className="text-sm font-medium text-brand-ink">הצגת כפתור עידן חדש למשתמשים</span>
      </label>
